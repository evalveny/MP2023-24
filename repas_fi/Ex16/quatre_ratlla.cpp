...
// DESCRIPTION: 
// Afegeix una fitxa del color del jugador que es passa com a paràmetre a una
// columna del taular 
// PARAMETERS:
// tauler: matriu que conté l'estat actual del joc
// columna: columna on es vol posar la fitxa (entre 0 i N_COLUMNES - 1)
// jugador: indica quin jugador està posant la fitxa. Pot tenir els valors 1 ó 2
// RETURN:
// - Si la fitxa no es pot posar perquè la columna ja està plena retorna -1
// - Si la fitxa sí que es pot posar retorna la fila on queda col·locada (suposant
//   que la fila superior és la fila 0. 
...
// DESCRIPTION: 
// Comprova si hi ha alguna combinació de fitxes (en qualsevol de les 4 direccions 
// possibles) que formi quatre en ratlla i que passi per la fila i columna que 
// s'indiquen com a paràmetre
// PARAMETERS:
// tauler: matriu que conté l'estat actual del joc
// fila: fila de la posició per la que es vol comprovar si forma 4 en ratlla
// columna: columna de la posició per la que es vol comprovar si forma 4 en ratlla
// RETURN:
